...
// Build a srcset string for an /uploads/*.webp URL. Variants are generated by the
// upload pipeline (400/800/1200). If the URL isn't a .webp, returns null and the
// browser falls back to `src` alone.
const buildSrcset = (url) => {
  if (!url || typeof url !== 'string') return null;
  const match = url.match(/^(.+)\.webp$/);
  if (!match) return null;
  const base = match[1];
  // Skip if it's already a sized variant (avoid -400-400.webp)
  if (/-\d+$/.test(base)) return null;
  return `${base}-400.webp 400w, ${base}-800.webp 800w, ${base}-1200.webp 1200w, ${url} 1600w`;
};

// Look up a base64 data URI for a portfolio image URL from window.__LP3_CRITICAL__.
// Returns null if not available (first paint before fetch resolves, or non-LP3 image).
const getCriticalDataUri = (url, critical) => {
  if (!Array.isArray(critical)) return null;
  for (let i = 0; i < critical.length; i++) {
    const c = critical[i];
    if (c && c.url === url && c.dataUri) return c.dataUri;
  }
  return null;
};
...
const LandingPage3 = () => {
  ...
  // Critical inlined images (base64) from window.__LP3_CRITICAL__, populated by the
  // bootstrap script in index.html. Subscribes to lp3-critical-ready in case the
  // fetch finishes after this component mounts.
  const [criticalImages, setCriticalImages] = useState(() => {
    if (typeof window !== 'undefined' && Array.isArray(window.__LP3_CRITICAL__)) {
      return window.__LP3_CRITICAL__;
    }
    return null;
  });
  useEffect(() => {
    const onReady = (e) => {
      if (Array.isArray(e?.detail)) setCriticalImages(e.detail);
      else if (typeof window !== 'undefined' && Array.isArray(window.__LP3_CRITICAL__)) {
        setCriticalImages(window.__LP3_CRITICAL__);
      }
    };
    window.addEventListener('lp3-critical-ready', onReady);
    return () => window.removeEventListener('lp3-critical-ready', onReady);
  }, []);
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: (typeof img === 'string' && !img.includes('placeholder')) ? 'transparent' : '#c4c4c4',
                        borderRadius: '20px',
                        border: borderStyle,
                    ...
                    >
                      {typeof img === 'string' && !img.includes('placeholder') ? (() => {
                        const dataUri = index < 3 ? getCriticalDataUri(img, criticalImages) : null;
                        const srcset = !dataUri ? buildSrcset(img) : null;
                        return (
                          <img
                            src={dataUri || img}
                            srcSet={srcset || undefined}
                            sizes={isCenter ? '138px' : '100px'}
                  ...
                  return (
                    ...
                    >
                      {typeof img === 'string' && !img.includes('placeholder') ? (() => {
                        const dataUri = index < 3 ? getCriticalDataUri(img, criticalImages) : null;
                        const srcset = !dataUri ? buildSrcset(img) : null;
                        const isAboveFold = Math.abs(index - currentSlide) <= 1;
                        return (
                          <img
                            src={dataUri || img}
                            srcSet={srcset || undefined}
                            sizes={isCenter ? '300px' : '254px'}
                            alt={`Portfolio ${index + 1}`}
                            width={300}
                            height={629}
