...
export default function GotiVideosDashboard() {
  ...
  const createFolder = () => {
    ...
    setFolders(f => [...f, { id: Date.now().toString(), name: newFolderName.trim() }]);
    setNewFolderName('');
  ...
  const confirmRename = () => {
    ...
    setFolders(f => f.map(x => x.id === renameTarget.id ? { ...x, name: renameName.trim() } : x));
  ...
  const getDisplayVideos = () => {
    let base = activeSubNav === 'recent'
      ? [...videos].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 10)
      : videos;
    return sortVideos(filterVideos(base));
  };
        ...
        {activeTab === 'media' && (
          <>
            ...
            {uploading && (
              <div style={{ padding: '8px 24px 0', flexShrink: 0 }}>
                ...
                <div style={{ color: '#6d7175', fontSize: '12px', marginTop: '4px', paddingBottom: '8px' }}>
                  Uploading... {uploadProgress}% — compression starts after upload
                </div>
              </div>
            )}
            ...
            <div style={{ padding: '12px 24px', borderBottom: '1px solid #f0f0f0', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowProductFilter(v => !v); setShowMediaTypeFilter(false); setShowSortMenu(false); }}
                    style={{
                      padding: '5px 12px', background: activeProductFilter ? '#e8f4f3' : '#fff',
                      border: `1px solid ${activeProductFilter ? '#0dbaab' : '#d1d5db'}`,
                      borderRadius: '20px', fontSize: '12px',
                      color: activeProductFilter ? '#0dbaab' : '#374151',
                  ...
                  {showProductFilter && (
                    <div onClick={e => e.stopPropagation()} style={{
                      ...
                      {['With tagged products', 'Without tagged products', 'With specific product'].map(opt => (
                ...
                <div style={{ position: 'relative' }}>
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowMediaTypeFilter(v => !v); setShowProductFilter(false); setShowSortMenu(false); }}
                    style={{
                      padding: '5px 12px', background: activeMediaType ? '#e8f4f3' : '#fff',
                      border: `1px solid ${activeMediaType ? '#0dbaab' : '#d1d5db'}`,
                      borderRadius: '20px', fontSize: '12px',
                      color: activeMediaType ? '#0dbaab' : '#374151',
                      cursor: 'pointer', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '4px'
                    }}>
                    Media Type {activeMediaType ? '✓' : '▾'}
                  </button>
                  {showMediaTypeFilter && (
                    <div onClick={e => e.stopPropagation()} style={{
                      position: 'absolute', top: '32px', left: 0, background: '#fff',
                      ...
                      {['image', 'video', 'instagram_video', 'instagram_image', 'instagram_stories', 'tiktok'].map((type, i) => {
                        const labels = { image: 'Image', video: 'Video', instagram_video: 'Instagram video posts', instagram_image: 'Instagram image posts', instagram_stories: 'Instagram stories', tiktok: 'Tiktok videos' };
                        return (
                ...
                <div style={{ marginLeft: 'auto', position: 'relative' }}>
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowSortMenu(v => !v); setShowProductFilter(false); setShowMediaTypeFilter(false); }}
                    style={{
                      display: 'flex', alignItems: 'center', gap: '6px',
                      background: 'transparent', border: 'none', cursor: 'pointer',
                      color: '#6d7175', fontSize: '13px', fontWeight: '500', padding: '5px 8px',
                      borderRadius: '6px'
                    }}>
                    {sortLabels[sortBy]} ↓
                  </button>
                    ...
                    <div onClick={e => e.stopPropagation()} style={{
                      ...
                      {Object.entries(sortLabels).map(([key, label], i) => (
                        <button key={key} onClick={() => { setSortBy(key); setShowSortMenu(false); }} style={{
                          width: '100%', padding: '9px 14px', background: '#fff',
                          border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: '13px',
                          color: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '16px' }}>
                    {folders.map(folder => (
                      ...
                      >
                        <div style={{
                          display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px 8px 10px',
                          borderRadius: '10px', border: '1px solid transparent',
                          background: hoveredFolder === folder.id ? '#f9fafb' : 'transparent',
                          transition: 'background 0.15s'
                        }}>
                          <FolderSVG size={72} />
                          <div style={{ marginTop: '8px', fontSize: '13px', fontWeight: '500', color: '#1a1a1a', textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%' }}>
                            {folder.name}
                          </div>
                        </div>
              ...
              {/* Videos */}
              {displayVideos.length === 0 && folders.length === 0 && !uploading ? (
      ...
      {showNewFolderModal && (
        <div
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ fontSize: '17px', fontWeight: '700', color: '#1a1a1a', marginBottom: '20px' }}>
              Create new folder
            </div>
